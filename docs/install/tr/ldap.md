# iRedPanel Kurulumu: iRedMail OpenLDAP Sunucusu

Bu doküman, OpenLDAP backend kullanan bir iRedMail sunucusu için iRedPanel kurulumunu anlatır. İki yöntem vardır: mail sunucusuna native kurulum ve Docker container.

Diğer backend'ler: [MariaDB/MySQL](mariadb.md), [PostgreSQL](postgresql.md). İngilizce sürüm: [../en/ldap.md](../en/ldap.md).

## Uyumluluk

| iRedPanel | Test edilen iRedMail sürümü | Test edilen sürüm türü |
|-----------|-----------------------------|------------------------|
| 1.0.2     | 1.7.4                       | OPENLDAP edition       |
...
iRedMail OpenLDAP kurulumunda mail hesapları LDAP'tadır. iRedAdmin, Amavisd ve iRedAPD verileri MariaDB'dedir. Bu yüzden panel ikisini de kullanır.

| Servis | Mail sunucusundaki varsayılan adres | Kullanım | Gerekli |
|--------|-------------------------------------|----------|---------|
| OpenLDAP | port `389` | Domain, mailbox, alias, admin | Evet |
| MariaDB `iredadmin` | `127.0.0.1:3306` | Activity log, panel ayarları, API key, domain sahipliği, silinen mailbox'lar, son girişler, kullanılan quota | Önerilir |
...
Newsletter sayfaları ve `cli/notifyQuarantinedRecipients.php` yalnızca SQL backend'leri destekler. Bu yüzden LDAP kurulumu SMTP ayarı gerektirmez.

Mail sunucusuna native kurulum her servise `127.0.0.1` üzerinden ulaşır. Docker container veya başka bir sunucu için [Ağ Erişimi](#ağ-erişimi) bölümünü uygulayın.
...
```bash
grep -E '^ldap_|^(iredadmin|amavisd|iredapd)_db_|^amavisd_quarantine_port|^mlmmjadmin_api_auth_token' /opt/www/iredadmin/settings.py
```

| `settings.py` key | iRedPanel değişkeni |
|-------------------|---------------------|
| `ldap_uri` | `IREDPANEL_LDAP_URI` |
| başındaki `o=domains,` olmadan `ldap_basedn` | `IREDPANEL_LDAP_ROOT_DN` |
| `ldap_bind_dn` | `IREDPANEL_LDAP_USER` |
| `ldap_bind_password` | `IREDPANEL_LDAP_PASSWORD` |
...
Örnek: `ldap_basedn = "o=domains,dc=example,dc=com"` değeri `IREDPANEL_LDAP_ROOT_DN=dc=example,dc=com` olur. Panel, mail domain'lerini bu kökün altındaki `o=domains` dalında, standalone admin'leri `o=domainAdmins` dalında okur.

`ldap_bind_dn` değeri `cn=vmailadmin,<root>` olur. Bu hesabın mail ağacına yazma yetkisi vardır. Panel, girişten sonraki her LDAP isteği ve CLI araçları için bu hesabı kullanır. Admin girişini ise o admin ile bind yaparak kontrol eder. Panel kendi tablolarını (`panel_settings`, `panel_api_keys`) ilk kullanımda `iredadmin` database'inde oluşturur.
...
```ini
IREDPANEL_BACKEND=ldap
# Generate with: openssl rand -hex 32
IREDPANEL_SECRET_KEY=<random string>

IREDPANEL_LDAP_URI=ldap://127.0.0.1:389
IREDPANEL_LDAP_ROOT_DN=dc=example,dc=com
IREDPANEL_LDAP_USER=cn=vmailadmin,dc=example,dc=com
IREDPANEL_LDAP_PASSWORD=<ldap_bind_password>
...
- `IREDPANEL_LDAP_URI`, `ldap://` ve `ldaps://` kabul eder. `IREDPANEL_LDAP_TLS_VERIFY=true` olduğunda panel `ldaps://` sunucusunun sertifikasını doğrular.
- LDAP backend ile entegrasyon database'leri varsayılan olarak `3306` port'unu kullanır.
- Kullanmadığınız özelliğin bloğunu (Amavisd, iRedAPD veya mlmmjadmin) yazmayın.
- LDAP backend maildir'leri `/var/vmail` altında okur. `IREDPANEL_VMAIL_PATH` ve `IREDPANEL_STORAGE_NODE` yalnızca SQL backend'lerinde geçerlidir.
...
LDAP backend, hesaplar için `ldap` ve MariaDB database'leri için `pdo_mysql` extension'ını ister. Debian veya Ubuntu üzerinde extension'ları ve Composer'ı kurun:

```bash
apt install php-ldap php-mysql php-mbstring composer git unzip
```

Başka bir dağıtımda, PHP-FPM'in PHP sürümü için `ldap` ve `pdo_mysql` sağlayan paketleri kurun. Composer'ı <https://getcomposer.org/> adresinden kurun.
...
Paneli başka bir sunucuda Apache ile çalıştırmak için `apache2`, `libapache2-mod-php`, `php-ldap`, `php-mysql` ve `php-mbstring` paketlerini kurun. `a2enmod rewrite` komutunu çalıştırın. Virtual host'u `AllowOverride All` ile `public/` dizinine yönlendirin. `public/.htaccess` URL'leri rewrite eder. Bu sunucu mail sunucusuna ağ üzerinden bağlanır, bu yüzden [Ağ Erişimi](#ağ-erişimi) bölümünü de uygulayın.
...
`.env.prod` dosyasını [Adım 2](#adım-2-yapılandırmayı-yazın) bölümündeki gibi düzenleyin. `IREDPANEL_LDAP_URI`, database host'ları ve mlmmjadmin URL'indeki host değerleri container'ın çalıştığı yere bağlıdır:
...
Debian ve Ubuntu'da `/etc/default/slapd` içindeki `SLAPD_SERVICES` değeri `slapd`'nin adreslerini belirler. `ldap:///` her adresi dinler. `ldap://127.0.0.1:389/` yalnızca loopback'i dinler. Bu durumda `ldap:///` veya mail sunucusunun adresini ekleyin ve `systemctl restart slapd` komutunu çalıştırın.

`ldap://` bind parolasını şifresiz gönderir. Bunu yalnızca mail sunucusunun kendisinde veya Docker bridge üzerinde kullanın. Başka bir sunucu için `IREDPANEL_LDAP_TLS_VERIFY=true` ile `ldaps://` kullanın veya özel bir ağ kullanın.
...
Mail sunucusunun firewall'unda, panel adresinden `389`, `3306`, `7790` ve `9998` port'larına TCP erişimine izin verin. Başka hiçbir adrese izin vermeyin. Bu port'ları internete açmayın.

## İlk Giriş

Panel URL'ini açın ve bir global admin ile giriş yapın. iRedMail installer, `postmaster@<ilk domain>` hesabını `domainGlobalAdmin=yes` ile oluşturur. Dashboard domain, kullanıcı ve admin sayılarını gösterir. **System > iRedMail Compatibility** sayfası, kurulu sürümün test edildiği iRedMail sürümlerini gösterir.

Sayfa "Configuration error. Check server logs." mesajını gösterirse, gerekli bir değişken eksiktir veya `ldap` extension'ı yüklü değildir. Nedeni PHP error log'unda yazar. Sayfa backend hata sayfasını gösterirse, panel OpenLDAP'a bağlanamıyordur.
...
`deleteExpiredMailboxes.php`, `/var/vmail` dizinine yazma izni ister. Storage node dizini (`vmail1`) yoksa script bütün kayıtları tutar, çünkü mount edilmemiş bir dizin her maildir'i silinmiş gibi gösterir. Script'i önce `--dry-run` ile çalıştırın.
