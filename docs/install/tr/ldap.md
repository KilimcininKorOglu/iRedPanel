...
Panel URL'ini açın ve bir global admin ile giriş yapın. iRedMail installer, `postmaster@<ilk domain>` hesabını `domainGlobalAdmin=yes` ile oluşturur. Dashboard domain, kullanıcı ve admin sayılarını gösterir. **System > iRedMail Compatibility** sayfası, kurulu sürümün uyumlu olduğu iRedMail sürümlerini gösterir.
