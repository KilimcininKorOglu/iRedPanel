# Installing iRedPanel on an iRedMail OpenLDAP Server

This guide installs iRedPanel for an iRedMail server that uses the OpenLDAP backend. It covers two methods: a native install on the mail server, and a Docker container.

Other backends: [MariaDB/MySQL](mariadb.md), [PostgreSQL](postgresql.md). Turkish version: [../tr/ldap.md](../tr/ldap.md).

## Compatibility

| iRedPanel | Tested iRedMail version | Tested edition   |
|-----------|-------------------------|------------------|
...
In an iRedMail OpenLDAP installation, the mail accounts live in LDAP, and the iRedAdmin, Amavisd and iRedAPD data live in MariaDB. The panel therefore uses both.

| Service | Default address on the mail server | Used for | Required |
|---------|------------------------------------|----------|----------|
| OpenLDAP | port `389` | Domains, mailboxes, aliases, admins | Yes |
| MariaDB `iredadmin` | `127.0.0.1:3306` | Activity log, panel settings, API keys, domain ownership, deleted mailboxes, last logins, used quota | Recommended |
...
The newsletter pages and `cli/notifyQuarantinedRecipients.php` support only the SQL backends, so an LDAP installation needs no SMTP settings.

A native install on the mail server reaches every service through `127.0.0.1`. A Docker container or another host needs the changes in [Network Access](#network-access).
...
| `ldap_basedn` without the leading `o=domains,` | `IREDPANEL_LDAP_ROOT_DN` |
...
Example: `ldap_basedn = "o=domains,dc=example,dc=com"` gives `IREDPANEL_LDAP_ROOT_DN=dc=example,dc=com`. The panel reads mail domains under `o=domains` and standalone admins under `o=domainAdmins` of that root.

`ldap_bind_dn` is `cn=vmailadmin,<root>`. This account has write access to the mail tree, and the panel uses it for every LDAP request after login and for the CLI tools. The panel then checks an admin login with a bind as that admin. The panel creates its own tables (`panel_settings`, `panel_api_keys`) in the `iredadmin` database on first use.
...
- `IREDPANEL_LDAP_URI` accepts `ldap://` and `ldaps://`. `IREDPANEL_LDAP_TLS_VERIFY=true` makes the panel verify the certificate of an `ldaps://` server.
- The integration databases use port `3306` by default with the LDAP backend.
- Leave out the Amavisd, iRedAPD or mlmmjadmin block when you do not use that feature.
- The LDAP backend reads maildirs under `/var/vmail`. `IREDPANEL_VMAIL_PATH` and `IREDPANEL_STORAGE_NODE` apply only to the SQL backends.
...
The LDAP backend needs `ldap` for the accounts and `pdo_mysql` for the MariaDB databases. Install the extensions and Composer on Debian or Ubuntu:

```bash
apt install php-ldap php-mysql php-mbstring composer git unzip
```

On another distribution, install the packages that provide `ldap` and `pdo_mysql` for the PHP version of PHP-FPM, and install Composer from <https://getcomposer.org/>.
...
To run the panel with Apache on another server, install `apache2`, `libapache2-mod-php`, `php-ldap`, `php-mysql` and `php-mbstring`, run `a2enmod rewrite`, and point the virtual host at `public/` with `AllowOverride All`. `public/.htaccess` rewrites the URLs. That server reaches the mail server over the network, so apply [Network Access](#network-access) as well.
...
Edit `.env.prod` as in [Step 2](#step-2-write-the-configuration). The host values in `IREDPANEL_LDAP_URI`, the database hosts and the mlmmjadmin URL depend on where the container runs:
...
On Debian and Ubuntu, `SLAPD_SERVICES` in `/etc/default/slapd` sets the addresses of `slapd`. `ldap:///` listens on every address. `ldap://127.0.0.1:389/` listens only on loopback; add `ldap:///` or the address of the mail server in that case and run `systemctl restart slapd`.

`ldap://` sends the bind password without encryption. Use it only on the mail server itself or on the Docker bridge. For another server, use `ldaps://` with `IREDPANEL_LDAP_TLS_VERIFY=true`, or a private network.
...
Allow TCP from the panel address to ports `389`, `3306`, `7790` and `9998` in the firewall of the mail server, and to no other address. Do not publish these ports to the internet.

## First Login

Open the panel URL and log in as a global admin. The iRedMail installer creates `postmaster@<first domain>` with `domainGlobalAdmin=yes`. The dashboard shows the domain, user and admin counts. **System > iRedMail Compatibility** shows the tested iRedMail versions of the installed release.

If the page shows "Configuration error. Check server logs.", a required variable is missing or the `ldap` extension is not loaded. The PHP error log names the cause. If the page shows the backend error page, the panel cannot reach OpenLDAP.
...
`deleteExpiredMailboxes.php` needs write access to `/var/vmail`. It keeps every record when the storage node directory (`vmail1`) is missing, because a missing mount makes every maildir look deleted. Run it with `--dry-run` first.
